...
<html>
  <head>
    ...
    <title>index_name_l3.jsp 3级指标名称维护</title>
	...
	<STYLE TYPE="text/css">
		.delete {	background-image: url(<%=request.getContextPath() %>/images/close.gif) !important; }  
	...
	<script type="text/javascript">
		Ext.onReady(function() {
			//创建表单存放日期和表格
			var inUpdForm = Ext.create("Ext.form.Panel", {
				id : "inUpdForm",
				title : "三级指标维护",
			...
			var l3UpdStore = new Ext.data.Store({
				//***最后加上参照的二级指标名称（未完成）
				fields : [ "id","parentIdL1","parentId", "zbName", "zbSort", "zbPoint", "zbWeight",
						"zbPrincipal", "zbTarget", "zbSource", "zbNow", "zbTime",
						"zbFrequentness" ],//"parentIdL1","parentId"不显示，也不用修改，但要传给后台修改。
				proxy : {
					type : "ajax",
			...
			var l3UpdColumns = [ 
				...
				{header:"二级指标名称",width:100,dataIndex:"parentId",renderer:getUpL2Name},
				{header : "三级指标名称",width : 150,dataIndex : "zbName",sortable : true,
					editor : {xtype : "textfield",allowBlank : true}
				},
				{header : "排序规则",width : 150,dataIndex : "zbSort",sortable : true,
					editor : {
						xtype : "numberfield"
					}
				},
				{header : "所占分数",width : 150,dataIndex : "zbPoint",sortable : true,
					editor : {xtype : "textfield",allowBlank : true}
				}, 
				{header : "权重",width : 150,dataIndex : "zbWeight",sortable : true,
					editor : {xtype : "textfield",allowBlank : true}
				}, 
				{header : "负责人",width : 150,dataIndex : "zbPrincipal",sortable : true,
					editor : {xtype : "textfield",allowBlank : true}
				},
				{header : "指标目标值",width : 150,dataIndex : "zbTarget",sortable : true,
					editor : {xtype : "textfield",allowBlank : true}
				}, 
				{header : "指标来源",width : 150,dataIndex : "zbSource",sortable : true,
					editor : {xtype : "textfield",allowBlank : true}
				},
				{header : "指标当前值",width : 150,dataIndex : "zbNow",sortable : true,
					editor : {xtype : "textfield",allowBlank : true}
				}, 
				{header : "排序规则",width : 150,dataIndex : "zbTime",sortable : true,
					editor : {xtype : "textfield",allowBlank : true}
				},
				{header : "指标考核频度",width : 150,dataIndex : "zbFrequentness",sortable : true,
					editor : {xtype : "textfield",allowBlank : true}
				} 
			];
			//选择日期触发事件
			...
			function getUpL2Name(value,metadata,record,rowIndex,colIndex,view){
				return (l3_2UpdStore.findRecord("id",value,0,true,false,false)).get("zbName");
			}
			//获取要修改的三级指标
								...
								l3UpdStore.load({
									...
									callback : function(records, operation, success) {
										...
										if (Ext.getCmp("l3UpdGrid") == null) {
											...
											inUpdForm.remove(Ext.getCmp("l3UpdGrid"));
											inUpdForm.add(
											//创建表格
											Ext.create("Ext.grid.Panel", {
												title : "维护三级指标（修改、删除）",
			...
			function submitEditL3(e) {//e代表所做的操作。
				...
				var requestConfig = {
					...
					params : {
						...
						zbName : editedRecord.get("zbName"),
						zbSort : editedRecord.get("zbSort"),
						zbPoint : editedRecord.get("zbPoint"),
						zbWeight : editedRecord.get("zbWeight"),
						zbPrincipal : editedRecord.get("zbPrincipal"),
						zbSource : editedRecord.get("zbSource"),
						zbTarget : editedRecord.get("zbTarget"),
						zbNow : editedRecord.get("zbNow"),
						zbTime : editedRecord.get("zbTime"),
						zbFrequentness : editedRecord.get("zbFrequentness")
					}
				/* ,
