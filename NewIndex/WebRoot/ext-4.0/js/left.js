...
var menuTree = Ext.create("Ext.tree.Panel", {
			...
			listeners:{ 
				...
				itemclick:function(view,rec,item,index,e){
					console.log("点击树中节点"+rec.get("id")+"还有几个子panel"+Ext.getCmp("center").items.length);
					//点中的节点的id
					...
					if(Ext.Array.contains(['inOneForm','inTwoForm','inThreeForm','inNaMatFormL1','inNaMatFormL2','inNaMatFormL3','data_reuse_form','usMatForm','inMatForm'],match_id)){
						if(show_index!=-1){//点击的节点在panel中存在
							...
							console.log("点击已存在panel.打开的叶子数量："+tab_id.length+"还有几个子panel"+Ext.getCmp("center").items.length);
							...
							console.log("新增panel.打开的叶子数量："+tab_id.length+"还有几个子panel"+Ext.getCmp("center").items.length);
						}
					}
