	Ext.QuickTips.init();
	var inOneForm = Ext.create("Ext.panel.Panel",{
		id:"inOneForm",
		title:"一级指标",
		width:1177,
		height:383,
		...
		/*listeners:{
			close:function(tabItem, e){
//				alert("删除的id是"+tabItem.id);
//				Ext.Array.remove(tab_id,tabItem.id);//在数组中，删除刚关闭的tab的id
				
				//删除tab_id中的元素
				for(var i=0;i<tab_id.length;i++){
					if(tab_id[i]==tabItem.id){
						for(var j=i+1;j<tab_id.length;j++){
							tab_id[j-1] = tab_id[j];
						}
						tab_id.length = tab_id.length-1;
						break;
					}
				}
...
//				center.remove(this);
				...
				console.log("关闭一个之后.打开的叶子数量："+tab_id.length+"还有几个子panel"+Ext.getCmp("center").items.length);
